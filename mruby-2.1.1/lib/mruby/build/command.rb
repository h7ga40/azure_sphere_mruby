...
module MRuby
  ...
  class Command::Compiler < Command
    ...
    def all_flags(_defines=[], _include_paths=[], _flags=[], params={})
    ...
    def run(outfile, infile, _defines=[], _include_paths=[], _flags=[])
      ...
      if MRUBY_BUILD_HOST_IS_CYGWIN
        _run compile_options, { :flags => all_flags(_defines, _include_paths, _flags, params),
  ...
  class Command::Linker < Command
    ...
    def run(outfile, objfiles, _libraries=[], _library_paths=[], _flags=[], _flags_before_libraries=[], _flags_after_libraries=[])
      ...
      if MRUBY_BUILD_HOST_IS_CYGWIN
        params = { :outfile => cygwin_filename(outfile), :outdir => File.dirname(outfile).gsub('/', @build.file_separator), :outfilebase => File.basename(outfile, ".*") }
        ...
        params = { :outfile => filename(outfile), :outdir => File.dirname(outfile).gsub('/', @build.file_separator), :outfilebase => File.basename(outfile, ".*") }
        _run link_options, { :flags => all_flags(_library_paths, _flags, params),
  ...
  class Command::Archiver < Command
    ...
    def run(outfile, objfiles)
      ...
      if MRUBY_BUILD_HOST_IS_CYGWIN
        _run archive_options, { :outfile => cygwin_filename(outfile), :objs => cygwin_filename(objfiles).map{|f| %Q["#{f}"]}.join(' '), :outdir => File.dirname(outfile).gsub('/', @build.file_separator), :outfilebase => File.basename(outfile, ".*") }
      else
        _run archive_options, { :outfile => filename(outfile), :objs => filename(objfiles).map{|f| %Q["#{f}"]}.join(' '), :outdir => File.dirname(outfile).gsub('/', @build.file_separator), :outfilebase => File.basename(outfile, ".*") }
      end
